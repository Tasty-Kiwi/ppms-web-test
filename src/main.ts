...
async function createRenderable(meshData: Mesh): Promise<THREE.LineSegments> {
  ...
  meshData.segments.forEach((segment) => {
    if (segment.length < 2)
      throw `Invalid segment detected: ${JSON.stringify(segment)}`;
    for (let i = 1; i < segment.length; i++) {
      if (meshData.vertexes[segment[i - 1]].length === 3)
        ...
        throw `Invalid vertex detected: ${JSON.stringify(
          meshData.vertexes[segment[i - 1]]
        )}`;
      if (meshData.vertexes[segment[i]].length === 3)
        points.push(...meshData.vertexes[segment[i]]);
      else if (meshData.vertexes[segment[i]].length === 2)
        points.push(...meshData.vertexes[segment[i]], 0);
      else
        throw `Invalid vertex detected: ${JSON.stringify(
          meshData.vertexes[segment[i]]
        )}`;
    }
